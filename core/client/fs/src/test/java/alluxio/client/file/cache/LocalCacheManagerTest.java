...
/*Spencer add*/
  ...
  public PageStoreOptions mPageStoreOptions;
  public LocalPageStoreOptions localPageStoreOptions;
  public MemoryPageStoreOptions MemoryPageStoreOptions;
  public RocksPageStoreOptions RocksPageStoreOptions;
...
//  private LocalPageStoreOptions mPageStoreOptions;
  ...
  private PageStore localPageStore;
  private PageStore RocksPageStore;
//  private PageStore MemoryPageStore;
//  private LocalPageStoreDir mPageStoreDir;
  ...
  public void before() throws Exception {
    ...
    //~~~~~~~~//
    localPageStoreOptions = (LocalPageStoreOptions) PageStoreOptions.create(mConf).get(0);
    MemoryPageStoreOptions = (MemoryPageStoreOptions) PageStoreOptions.create(mConf).get(0);
    RocksPageStoreOptions = (RocksPageStoreOptions) PageStoreOptions.create(mConf).get(0);
    //~~~~~//
    localPageStore = PageStore.create(localPageStoreOptions);
    RocksPageStore = PageStore.create(RocksPageStoreOptions);
//    MemoryPageStore = PageStore.create(MemoryPageStoreOptions);
...
//    mPageStoreDir = new LocalPageStoreDir(mPageStoreOptions, mPageStore, mEvictor);
  ...
  private LocalCacheManager createLocalCacheManager() throws Exception {
    mPageStoreOptions = PageStoreOptions.create(mConf).get(0);
    if(mPageStoreOptions.getType().equals(PageStoreType.MEM)){

      mPageStoreOptions = PageStoreOptions.create(mConf).get(0); //
      mPageStoreDir = new MemoryPageStoreDir(mPageStoreOptions,
          (MemoryPageStore) PageStore.create(mPageStoreOptions), mEvictor);
      return createLocalCacheManagerForMEM(mConf, mPageMetaStore, ImmutableList.of(mPageStoreDir));

    }else if(mPageStoreOptions.getType().equals(PageStoreType.LOCAL)){

      mPageStore = PageStore.create(mPageStoreOptions);
      mPageStoreDir = new LocalPageStoreDir(localPageStoreOptions, localPageStore, mEvictor);
      mEvictor = new FIFOCacheEvictor(mConf);
      mPageMetaStore = new DefaultPageMetaStore(ImmutableList.of(mPageStoreDir));
      return createLocalCacheManager(mConf, mPageMetaStore);

    }else if(mPageStoreOptions.getType().equals(PageStoreType.ROCKS)){

      mPageStore = PageStore.create(mPageStoreOptions);
      mPageStoreDir = new RocksPageStoreDir(RocksPageStoreOptions, RocksPageStore, mEvictor);
      mEvictor = new FIFOCacheEvictor(mConf);
      mPageMetaStore = new DefaultPageMetaStore(ImmutableList.of(mPageStoreDir));
      return createLocalCacheManager(mConf, mPageMetaStore);
...
//    return createLocalCacheManager(mConf, mPageMetaStore);
  ...
  private LocalCacheManager createLocalCacheManagerForMEM(AlluxioConfiguration conf,
      PageMetaStore pageMetaStore,
      List<PageStoreDir> dirs) throws Exception {
    LocalCacheManager cacheManager = LocalCacheManager.create(conf, pageMetaStore, dirs);
    CommonUtils.waitFor("restore completed",
        () -> cacheManager.state() == CacheManager.State.READ_WRITE,
        WaitForOptions.defaults().setTimeoutMs(10000));
    return cacheManager; // 参数之一是 list<PageStoreDir>
  }
